...
describe('Listbox', () => {
  ...
  describe('Keyboard Navigation', () => {
    ...
    test('Enter key selects active item in multi-select mode', async () => {
      ...
      await expect
        .poll(() => page.getByTestId('apple').element().getAttribute('aria-selected'))
        .toBe('true')
    })
  ...
  describe('Filtering', () => {
    test('query filters visible items', async () => {
      ...
      await expect
        .poll(() => (page.getByTestId('apple').element() as HTMLElement).hidden)
        .toBe(true)
      await expect
        .poll(() => (page.getByTestId('banana').element() as HTMLElement).hidden)
        .toBe(false)
    })
