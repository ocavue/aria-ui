...
const getProject = memoize(getProjectImpl)
 ...
 */
function parseImpl(tsconfigFilePath: string, entryFilePath: string): ComponentInfo[] {
 ...
 */
function extractPropertiesFromInterface(interfaceDecl: InterfaceDeclaration): PropInfo[] {
 ...
 */
function extractEventsFromInterface(interfaceDecl: InterfaceDeclaration): EventInfo[] {
 ...
 */
function extractInterfaceProperties(interfaceDecl: InterfaceDeclaration): Array<EventInfo> {
  ...
  for (const property of properties) {
    ...
    const comment = jsDocComments.map((jsDocComment) => jsDocComment.getInnerText()).join('\n')
